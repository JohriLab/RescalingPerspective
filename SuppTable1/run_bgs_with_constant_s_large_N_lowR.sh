...
scaling_factor=1000
folder="/work/users/p/j/pjohri/RescaleReview/bgs_sims_large_N_lowR/C_${scaling_factor}"

if [ ! -f "${folder}/output${repID}_sample100.txt" ]
then
        slim -d d_seed=${repID} -d d_scale=${scaling_factor} -d d_L=1e6 -d "d_folder='${folder}'" -d "d_repID='${repID}'" bgs_with_constant_s_large_N_lowR.slim
fi
